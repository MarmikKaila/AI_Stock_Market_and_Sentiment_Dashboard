...
const Dashboard = () => {
  ...
  useEffect(() => {
    const loadStockData = async () => {
      try {
        setIsLoading(true);
        // Data is hardcoded to fetch for 'AAPL'
        const data = await getStockDetails('AAPL');
  ...
  if (isLoading) {
    return (
      <div className="bg-black min-h-screen">
        <Navbar />
        <div className="text-center text-white p-10">Loading data...</div>
      </div>
    );
  }
  ...
  const { stock, opinion } = stockDetails;
      ...
      <div className="p-6 grid grid-cols-1 lg:grid-cols-3 gap-6 text-white">
        ...
        <div className="col-span-1 space-y-4">
          <div className="bg-gray-900 p-6 rounded-xl h-48 flex flex-col">
            ...
            <p className="text-2xl font-bold">{stock.sentimentScore.toFixed(2)}</p>
          ...
          <div className="bg-gray-900 p-6 rounded-xl">
            ...
            <p className={`text-3xl font-bold mt-2 ${getRecommendationClass(opinion.recommendation)}`}>
